...
class App:
    def __init__(self):
        ...
        self.tracks = []
        self.hset, self.vset = 640, 480
        self.cam =  PiCamera() #cv2.VideoCapture("C:/Year4/EEM449 Final Year Project/Videos/experiment51.h264")
        #self.cap = cv2.VideoCapture(0)         #for webcam
        #self.cam.set(3,self.hset)                   #for webcam
        #self.cam.set(4,self.vset)                    #for webcam
        self.cam.resolution = (self.hset, self.vset)       #for PiCamera module
        #self.cam.framerate = 32                #for webcam
        self.rawCapture = PiRGBArray(self.cam, size=(self.hset, self.vset))    #for PiCamera module
        self.frame_idx = 0
        ...
        #Variables for multithreaded program control
        self.readyfornextframe=0
        self.captured=0
        self.endbyuser=0
	...
	# and occupied/unoccupied text
            ...
            #(grabbed, image) = self.cam.read() #for webcam or video feed
            #if not grabbed:
            #    break
            self.imagebuff = frame.array                 #for PiCamera module
            self.frame_count+=self.frame_count
            while self.readyfornextframe==0:
                self.captured=0
                if self.endbyuser==1:
                    break
            self.image = self.imagebuff
            self.captured=1
    ...
    def estimate(self):
        while 1:
            #check user input 1st
            ch = 0xFF & cv2.waitKey(1)
            if ch == 27 or ch ==ord("q"):
                self.endbyuser=1
                break
            if ch ==ord("r"):
                self.absolute_y=self.absolute_yy=0
            #then proceed to program execution    
            self.readyfornextframe=1
            if self.captured==0:
                continue
            self.readyfornextframe=0
            height, width,channel = self.image.shape
            size=180
            frame_crop = self.image[(height/2):(height/2+size*2),(width/2-size):(width/2+size)]
            if self.first_pic==0:
...
def main():
    ...
    try:
        thread.start_new_thread(App().capture(), ("Thread-1",) )
        thread.start_new_thread(App().estimate(), ("Thread-2",) )
    except:
